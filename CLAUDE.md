...
- **`~/IdeaProjects/FABRIC_PORTING_PLAYBOOK.md`** — **v2 (2026-07-21): the consolidated knowledge base from all 14 ports + the multiplayer rollout** (26.1 drift dictionary, Create Fly architecture, render pipeline, dual-loader recipe, yarn-rename pipeline, the MP raw-id desync class, gametest harness, ops/packwiz, multi-agent process). Versioned mirror: `create26-ports/PORTING_PLAYBOOK.md` — update BOTH. **Read Parts 0–1 before ANY work; read the task-matching part before touching code.**
...
| KubeJS | ⚠ 26.1.2 is NeoForge-ONLY (checked 07-04) | Modrinth — waiting on a Fabric build; until then pack tweaks = datapacks/overrides |
...
| Mod | Notes |
|---|---|
| ImmediatelyFast | 26.1.2 fabric ✓ |
| ModernFix | ⚠ 26.1.2 is NeoForge-only (07-04) — excluded until a Fabric build |
| Sodium, Lithium, FerriteCore | ✓ verified 07-04: sodium 0.9.1-beta.3, lithium 0.24.6, ferrite-core 9.0.0 (slug is `ferrite-core`) |
...
### Addon survey — candidates beyond the completed roadmap (re-checked 2026-07-12: Modrinth sweep — NO new Fabric 26.x builds in this table; KubeJS 8.0.3 & ModernFix 5.27.18 ship 26.1.2 but NeoForge-ONLY)
...
| Big Cannons (#8) | 1.21.1 neoforge+fabric | contraption physics + renderers — riskiest work type | ✅ **DONE 2026-07-12** (91 gametests, pack-integrated; Prism pass pending) |
...
~/IdeaProjects/
  ...
  create26-ports/            # workspace: Create addon ports + pack (Kotlin DSL, wrapper 9.5+)
    ...
    pack/
      ...
      tools/build_manifest.py# Modrinth resolver: slugs -> pinned index with hashes; fails loudly on pack rot
      overrides/mods/        # in-house jars ship here in the .mrpack (no public URLs)
      kubejs/  config/       # BLOCKED: KubeJS 26.1.2 is NeoForge-only — tweaks via datapacks until it ships
```
