...
import appeng.blockentity.misc.CondenserFluidHandler;
import appeng.blockentity.misc.GrowthAcceleratorBlockEntity;
import appeng.blockentity.misc.InscriberBlockEntity;
import appeng.blockentity.powersink.AEBasePoweredBlockEntity;
import appeng.blockentity.storage.MEChestBlockEntity;
import appeng.blockentity.storage.MEChestFluidHandler;
...
import appeng.neoforge.AENeoForgeCapabilities;
...
public final class InitCapabilityProviders {
    ...
    public static void register(RegisterCapabilitiesEvent event) {
        ...
        for (var type : AEBlockEntities.getSubclassesOf(AEBaseInvBlockEntity.class)) {
            event.registerBlockEntity(Capabilities.Item.BLOCK, type,
                    InitCapabilityProviders::getExposedItemHandler);
        }
        for (var type : AEBlockEntities.getSubclassesOf(AEBasePoweredBlockEntity.class)) {
            event.registerBlockEntity(Capabilities.Energy.BLOCK, type,
                    InitCapabilityProviders::getEnergyStorage);
        }
        for (var type : AEBlockEntities.getImplementorsOf(IInWorldGridNodeHost.class)) {
            event.registerBlockEntity(AENeoForgeCapabilities.IN_WORLD_GRID_NODE_HOST, type,
                    (object, context) -> (IInWorldGridNodeHost) object);
        }
    }

    @Nullable
    private static ResourceHandler<ItemResource> getExposedItemHandler(AEBaseInvBlockEntity blockEntity,
            @Nullable Direction side) {
        var exposed = blockEntity.getExposedInventory(side);
        return exposed == null ? null : NeoForgeResources.toResourceHandler(exposed);
    }

    @Nullable
    private static EnergyHandler getEnergyStorage(AEBasePoweredBlockEntity blockEntity, @Nullable Direction side) {
        if (!blockEntity.isExternalPowerSide(side)) {
            return null;
        }
        return (EnergyHandler) blockEntity.getOrCreateEnergyAdapter(ForgeEnergyAdapter::new);
    }
    ...
    public static void registerGenericAdapters(RegisterCapabilitiesEvent event) {
        ...
        for (var block : BuiltInRegistries.BLOCK) {
            if (event.isBlockRegistered(AENeoForgeCapabilities.GENERIC_INTERNAL_INV, block)) {
    ...
    private static <T> void registerGenericInvAdapter(RegisterCapabilitiesEvent event,
        ...
        event.registerBlock(
                ...
                (level, pos, state, blockEntity, context) -> {
                    var genericInv = level.getCapability(AENeoForgeCapabilities.GENERIC_INTERNAL_INV, pos, state,
                            blockEntity, context);
                    if (genericInv != null) {
    ...
    private static void initInterface(RegisterCapabilitiesEvent event) {
        event.registerBlockEntity(
                AENeoForgeCapabilities.GENERIC_INTERNAL_INV,
                AEBlockEntities.INTERFACE.get(),
                (be, context) -> be.getInterfaceLogic().getStorage());

        event.registerBlockEntity(
                AENeoForgeCapabilities.ME_STORAGE,
                AEBlockEntities.INTERFACE.get(),
                (blockEntity, context) -> {
    ...
    private static void initCondenser(RegisterCapabilitiesEvent event) {
        ...
        event.registerBlockEntity(Capabilities.Item.BLOCK, AEBlockEntities.CONDENSER.get(),
                (blockEntity, context) -> {
                    return NeoForgeResources.toResourceHandler(blockEntity.getExternalInv());
                });
        event.registerBlockEntity(Capabilities.Fluid.BLOCK, AEBlockEntities.CONDENSER.get(),
                ((blockEntity, context) -> {
                    return new CondenserFluidHandler(blockEntity);
                }));
        event.registerBlockEntity(AENeoForgeCapabilities.ME_STORAGE, AEBlockEntities.CONDENSER.get(),
                (blockEntity, context) -> {
    ...
    private static void initMEChest(RegisterCapabilitiesEvent event) {
        event.registerBlockEntity(Capabilities.Fluid.BLOCK, AEBlockEntities.ME_CHEST.get(),
                InitCapabilityProviders::getMEChestFluidHandler);
        event.registerBlockEntity(AENeoForgeCapabilities.ME_STORAGE, AEBlockEntities.ME_CHEST.get(),
                MEChestBlockEntity::getMEStorage);
    }

    @SuppressWarnings("unchecked")
    @Nullable
    private static ResourceHandler<FluidResource> getMEChestFluidHandler(MEChestBlockEntity blockEntity,
            Direction side) {
        return (ResourceHandler<FluidResource>) blockEntity.getFluidHandler(side, MEChestFluidHandler::new);
    }

    private static void initMisc(RegisterCapabilitiesEvent event) {
        event.registerBlockEntity(
                AENeoForgeCapabilities.CRAFTING_MACHINE,
                AEBlockEntities.MOLECULAR_ASSEMBLER.get(),
                (object, context) -> object);
        event.registerBlockEntity(
                Capabilities.Item.BLOCK,
                AEBlockEntities.DEBUG_ITEM_GEN.get(),
                (object, context) -> NeoForgeResources.toResourceHandler(object.getInventory()));
        event.registerBlockEntity(
                Capabilities.Energy.BLOCK,
                AEBlockEntities.DEBUG_ENERGY_GEN.get(),
                // The debug energy generator is an infinite source that accepts no insertion,
                // it used to implement EnergyHandler itself with the same semantics
                (object, context) -> InfiniteEnergyHandler.INSTANCE);
        event.registerBlockEntity(
                Capabilities.Fluid.BLOCK,
                AEBlockEntities.SKY_STONE_TANK.get(),
                (object, context) -> SkyStoneTankFluidHandler.get(object));
    }
    ...
    private static void initCrankable(RegisterCapabilitiesEvent event) {
        event.registerBlockEntity(AENeoForgeCapabilities.CRANKABLE, AEBlockEntities.CHARGER.get(),
                ChargerBlockEntity::getCrankable);
        event.registerBlockEntity(AENeoForgeCapabilities.CRANKABLE, AEBlockEntities.INSCRIBER.get(),
                InscriberBlockEntity::getCrankable);
        event.registerBlockEntity(AENeoForgeCapabilities.CRANKABLE, AEBlockEntities.GROWTH_ACCELERATOR.get(),
    ...
    private static void registerPartCapabilities(RegisterPartCapabilitiesEvent event) {
        event.register(Capabilities.Item.BLOCK,
                (part, direction) -> NeoForgeResources.toResourceHandler(part.getLogic().getBlankPatternInv()),
                PatternEncodingTerminalPart.class);
        event.register(AENeoForgeCapabilities.GENERIC_INTERNAL_INV, (part, context) -> part.getLogic().getReturnInv(),
                PatternProviderPart.class);
        event.register(AENeoForgeCapabilities.GENERIC_INTERNAL_INV,
                (part, context) -> part.getInterfaceLogic().getStorage(),
                InterfacePart.class);
        event.register(AENeoForgeCapabilities.ME_STORAGE,
        ...
        event.register(Capabilities.Energy.BLOCK,
                (part, context) -> (EnergyHandler) part.getOrCreateEnergyAdapter(ForgeEnergyAdapter::new),
