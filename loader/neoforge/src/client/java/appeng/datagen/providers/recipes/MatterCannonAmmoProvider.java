...
public class MatterCannonAmmoProvider extends AE2RecipeProvider {
    ...
    public void buildRecipes() {
        ...
        tagAmmo(items, output, AppEng.makeId("matter_cannon/nuggets/iron"),
                ConventionTags.IRON_NUGGET, 55.845f);
        tagAmmo(items, output, AppEng.makeId("matter_cannon/nuggets/gold"),
    ...
    private void tag(RecipeOutput output, String recipeId, String tagId, float weight) {
        tagAmmo(items, output,
                AppEng.makeId("matter_cannon/" + recipeId),
                TagKey.create(Registries.ITEM, Identifier.parse(tagId)), weight);
    }

    /**
     * Registers a tag-based ammo recipe that is only loaded when the tag is non-empty. This used to be
     * {@code MatterCannonAmmo.ammo(HolderGetter, RecipeOutput, Identifier, TagKey, float)} before the shared recipe
     * class was decoupled from the NeoForge recipe-condition types.
     */
    private static void tagAmmo(HolderGetter<Item> items, RecipeOutput consumer, Identifier id, TagKey<Item> tag,
            float weight) {
