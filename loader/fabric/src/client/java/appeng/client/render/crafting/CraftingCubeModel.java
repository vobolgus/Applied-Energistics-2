 ...
 */
public abstract class CraftingCubeModel implements FabricDynamicBlockStateModel {
    ...
    public void collectParts(BlockAndTintGetter level, BlockPos pos, BlockState state, RandomSource random,
        ...
        var modelData = ((net.fabricmc.fabric.api.blockgetter.v2.FabricBlockGetter) level)
                .getBlockEntityRenderData(pos) instanceof CraftingCubeModelData data
                        ? data
                        : null;

        // Retrieve the cube connection state from the render data
        // If none is present, just assume there are no adjacent crafting cube blocks
        EnumSet<Direction> connections = modelData != null ? modelData.connections()
        ...
        for (var cullFace : Direction.values()) {
            ...
            this.addInnerCube(cullFace, state, modelData, builder, x1, y1, z1, x2, y2, z2);
        }
    ...
    protected abstract void addInnerCube(Direction facing, BlockState state,
            @Nullable CraftingCubeModelData modelData, CubeBuilder builder,
