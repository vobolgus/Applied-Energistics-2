...
// Fabric twin of the NeoForge class with the same FQN (see PORTING_NOTES Phase 3a):
// DynamicBlockStateModel -> FabricDynamicBlockStateModel, NeoForge render data -> FabricBlockGetter,
// NeoForge CustomUnbakedBlockStateModel/QuadTransforms/ComposedModelState -> Fabric equivalents.
...
import appeng.fabric.client.render.FabricDynamicBlockStateModel;

/**
 * Model class for the connected texture glass model.
 */
public class QuartzGlassModel implements FabricDynamicBlockStateModel {
    // NOTE (fabric): NeoForge's twin declares a write-only ModelProperty GLASS_STATE here, which is only
    // handed to framedblocks via InterModComms on NeoForge; there is no Fabric equivalent.
    ...
    private BakedQuad createQuad(Direction side, Vector3f c1, Vector3f c2, Vector3f c3, Vector3f c4,
        ...
        // Was NeoForge's QuadBakingVertexConsumer; the vanilla BakedQuad is built directly instead
        // (vanilla quads carry neither per-vertex colors nor normals - the values the NeoForge twin
        // pushes for them are the defaults anyway: white and the face normal).
        var texture = sprite.sprite();
        var transparency = texture.contents().transparency();
        var itemRenderType = transparency.hasTranslucent() ? Sheets.translucentBlockItemSheet()
        ...
        return new BakedQuad(
                new Vector3f(c1), new Vector3f(c2), new Vector3f(c3), new Vector3f(c4),
                UVPair.pack(texture.getU(u1), texture.getV(v1)),
                UVPair.pack(texture.getU(u1), texture.getV(v2)),
                UVPair.pack(texture.getU(u2), texture.getV(v2)),
                UVPair.pack(texture.getU(u2), texture.getV(v1)),
                side,
                materialInfo);
    }
