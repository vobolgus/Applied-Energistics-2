 ...
 * <p>
 * Fabric twin of the NeoForge class with the same FQN: NeoForge's world-aware
 * {@code DynamicBlockStateModel#collectParts} becomes an override of the FRAPI-injected
 * {@code FabricBlockStateModel#emitQuads}; render data is read through {@code FabricBlockGetter}, and the cable quads
 * (whose baked per-vertex colors vanilla quads cannot carry) are re-colored from the {@link QuadColors} side channel
 * while being re-emitted into the mesh.
 */
public class CableBusModel implements BlockStateModel {
    ...
    public void collectParts(RandomSource random, List<BlockStateModelPart> parts) {
        // Without level access there is no render data and thus nothing to display
        // (mirrors NeoForge, where the context-free path passes an EMPTY level and finds no model data).
    }

    @Override
    public void emitQuads(QuadEmitter emitter, BlockAndTintGetter blockView, BlockPos pos, BlockState state,
            RandomSource random, Predicate<Direction> cullTest) {
        if (!(((FabricBlockGetter) blockView)
                .getBlockEntityRenderData(pos) instanceof CableBusRenderState renderState)) {
            return;
        }

        // First, handle the cable at the center of the cable bus. Its quads carry baked colors
        // through the QuadColors side channel and need a color-aware emission.
        var cableModel = cableModelCache.getUnchecked(renderState);
        if (cableModel != emptyCableModel) {
            emitColoredQuads(emitter, cableModel, cullTest);
        }
        ...
        // Facade quads carry pre-baked tints through the QuadColors side channel as well
        this.facadeBuilder.collectFacadeParts(
                renderState,
                blockView,
                part -> {
                    if (part instanceof FacadeBuilder.FacadeBlockModelPart facadePart) {
                        emitColoredQuads(emitter, facadePart, cullTest);
                    } else {
                        part.emitQuads(emitter, cullTest);
                    }
                });
    ...
    /**
     * Re-emits a part's quads, applying baked colors from the {@link QuadColors} side channel (vanilla BakedQuads
     * cannot carry per-vertex colors; the FRAPI mesh can).
     */
    private static void emitColoredQuads(QuadEmitter emitter, BlockStateModelPart part,
    ...
    public Object createGeometryKey(BlockAndTintGetter blockView, BlockPos pos, BlockState state,
            RandomSource random) {
        // Geometry depends entirely on the render data; identical render states produce identical geometry.
        if (((FabricBlockGetter) blockView).getBlockEntityRenderData(pos) instanceof CableBusRenderState renderState) {
            return renderState;
        }
        return null;
    }
