...
/**
 * Fabric twin of the NeoForge class with the same FQN; NeoForge's {@code StandaloneModelKey} becomes Fabric's
 * {@link ExtraModelKey} (registered by the AE2 Fabric client entrypoint's {@code ModelLoadingPlugin}).
 */
public class CrankRenderer implements BlockEntityRenderer<CrankBlockEntity, CrankRenderState> {
    ...
    public static final ExtraModelKey<BlockStateModelPart> HANDLE_MODEL = ExtraModelKey
            .create(HANDLE_MODEL_ID::toString);

    private final BlockStateModel handleModel;

    public CrankRenderer(BlockEntityRendererProvider.Context context) {
        var modelManager = Minecraft.getInstance().getModelManager();
        handleModel = new SingleVariant(Objects.requireNonNull(modelManager.getModel(HANDLE_MODEL)));
    }
