 ...
 * <p>
 * Fabric twin of the NeoForge class with the same FQN: the NeoForge {@code ExtractLevelRenderStateEvent} /
 * {@code RenderLevelStageEvent.AfterWeather} pair maps to {@code LevelRenderEvents.END_EXTRACTION} (render data is
 * attached to the {@code LevelRenderState} via the injected {@code FabricRenderState}) and
 * {@code LevelRenderEvents.END_MAIN} (the closest "world last" render stage).
 */
    ...
    public void install() {
        LevelRenderEvents.END_EXTRACTION.register(context -> {
            ...
            for (var visibleArea : visibleAreas) {
                if (visibleArea.getOverlaySourceLocation().getLevel() == context.level()) {
                    areasInThisLevel.add(visibleArea);
                }
            }
            context.levelState().setData(OVERLAY_AREAS, areasInThisLevel);
        });

        LevelRenderEvents.END_MAIN.register(context -> {
            var visibleAreas = context.levelState().getDataOrDefault(OVERLAY_AREAS, List.<IAreaOverlayDataSource>of());
            ...
            Vec3 projectedView = context.levelState().cameraRenderState.pos;
