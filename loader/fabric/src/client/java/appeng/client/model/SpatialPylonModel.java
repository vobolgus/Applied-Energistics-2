 ...
 */
public class SpatialPylonModel implements FabricDynamicBlockStateModel {
    ...
    public void collectParts(BlockAndTintGetter level, BlockPos pos, BlockState blockState, RandomSource random,
        ...
        var state = ((net.fabricmc.fabric.api.blockgetter.v2.FabricBlockGetter) level)
                .getBlockEntityRenderData(pos) instanceof SpatialPylonBlockEntity.ClientState clientState
                        ? clientState
    ...
    // NOTE (fabric): the world-aware materialFlags overload is the FRAPI-injected variant, which also takes the
    // RandomSource (NeoForge's patched overload does not).
    @Override
    public @BakedQuad.MaterialFlags int materialFlags(BlockAndTintGetter level, BlockPos pos, BlockState state,
        ...
        // location
        return FabricDynamicBlockStateModel.super.materialFlags(level, pos, state, random);
    }
